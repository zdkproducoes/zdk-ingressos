...
export function Hero({
  ...
  return (
    <>
      ...
      <section className="relative w-full overflow-hidden bg-wine-900">
        <Image
        ...
        <div className="absolute bottom-0 left-0 right-0 h-[30%] bg-gradient-to-b from-transparent to-wine-800 pointer-events-none" />
      </section>

      {/* Tarja-CTA logo abaixo */}
      <section
        className="relative bg-gradient-to-br from-wine-600 to-mauve-600
                   border-t-4 border-b-4 border-amber-sacode-400
                   py-7 px-5 text-center"
      >
        <span className="absolute left-4 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-amber-sacode-400" />
        <span className="absolute right-4 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-amber-sacode-400" />
        ...
        <div className="max-w-[1000px] mx-auto flex flex-wrap items-center justify-center gap-4 sm:gap-x-8">
          {vendasAbertas ? (
            <div className="font-display text-xl tracking-[0.08em] text-cream-100">
              Vendas <strong className="text-amber-sacode-300">ABERTAS</strong>
              <span className="mx-2 opacity-50">|</span>
              A partir de <strong className="text-amber-sacode-300">R$ {precoFmt}</strong>
            </div>
          ) : (
            <div className="font-display text-xl tracking-[0.08em] text-cream-100">
              Abertura das Vendas <strong className="text-amber-sacode-300">dia {avisoAbertura}</strong>
            </div>
          )}
          <a
            href="#ingressos"
            className="bg-amber-sacode-400 text-wine-900 px-8 py-3.5
