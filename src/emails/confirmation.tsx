...
export function renderConfirmationEmail(params: { firstName: string; confirmUrl: string }): string {
  ...
  return `<!DOCTYPE html>
<html lang="pt-BR"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1.0" /><title>Confirme seu cadastro — ${platform.name}</title></head>
...
<p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#C9CBCE;">Recebemos seu cadastro na plataforma ${escape(platform.name)}. Para concluir, confirme seu e-mail clicando no botão abaixo:</p>
<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto;"><tr><td style="background:#D9A63F;border-radius:8px;">
<a href="${confirmUrl}" style="display:inline-block;padding:14px 32px;font-size:16px;font-weight:600;color:#17130A;text-decoration:none;">Confirmar meu e-mail</a>
</td></tr></table>
...
<p style="margin:24px 0 0;padding:16px;background:#1D2029;border-radius:8px;font-size:14px;color:#A6A8AB;">⏱️ Este link expira em <strong style="color:#F4F4F2;">24 horas</strong>. Se não foi você, pode ignorar.</p>
</td></tr>
