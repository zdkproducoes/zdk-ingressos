...
<h1 style="margin:0;font-size:28px;font-weight:800;color:#F4F4F2;">🎟️ Você recebeu um ingresso!</h1>
</td></tr>
<tr><td style="padding:40px;">
<h2 style="margin:0 0 8px;font-size:22px;color:#F4F4F2;">Boa, ${esc(recipientFirstName)}! 🎉</h2>
<p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#C9CBCE;">
<strong style="color:#F4F4F2;">${esc(senderName)}</strong> transferiu um ingresso para você.
...
<p style="margin:0 0 4px;font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#A6A8AB;">${esc(batchName)}</p>
<p style="margin:0 0 16px;font-size:18px;font-weight:700;color:#F4F4F2;">${esc(attendeeName)}</p>
...
<p style="margin:12px 0 0;font-size:11px;color:#A6A8AB;text-align:center;font-family:monospace;">${esc(qrToken)}</p>
</td></tr></table>
<p style="margin:24px 0 4px;font-size:12px;color:#A6A8AB;text-align:center;">O ingresso também fica disponível na sua conta</p>
...
⚠️ <strong style="color:#F4F4F2;">Importante:</strong> não compartilhe este QR Code — uso único na entrada.
Ingressos recebidos por transferência <strong style="color:#F4F4F2;">não podem ser transferidos novamente</strong>.
</p>
</td></tr>
<tr><td style="padding:24px 40px;background:#08090C;text-align:center;border-top:1px solid #2A2F3B;">
<p style="margin:0;font-size:12px;color:#A6A8AB;">© ${new Date().getFullYear()} ${esc(platform.name)}</p>
